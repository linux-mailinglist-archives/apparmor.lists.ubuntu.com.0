...
Received: from mail.lfdr.de
	...
	id ox0gHLe/VmowAwEAu9opvQ
	(envelope-from <apparmor-bounces@lists.ubuntu.com>)
	for <lists+apparmor@lfdr.de>; Wed, 15 Jul 2026 01:01:11 +0200
X-Original-To: lists+apparmor@lfdr.de
Received: from lists.ubuntu.com (lists.ubuntu.com [185.125.189.65])
	by mail.lfdr.de (Postfix) with ESMTPS id 0C72B7594F5
	for <lists+apparmor@lfdr.de>; Wed, 15 Jul 2026 01:01:11 +0200 (CEST)
Authentication-Results: mail.lfdr.de;
	dkim=fail ("headers rsa verify failed") header.d=launchpad.net header.s=20210803 header.b=itDyyT4H;
...
Received: from localhost ([127.0.0.1] helo=lists.ubuntu.com)
	...
	(envelope-from <apparmor-bounces@lists.ubuntu.com>)
	id 1wjm7M-00051l-Gl; Tue, 14 Jul 2026 23:00:48 +0000
Received: from smtp-relay-services-1.internal ([10.131.114.214]
 ...
 id 1wjm7J-000513-A5
 for apparmor@lists.ubuntu.com; Tue, 14 Jul 2026 23:00:45 +0000
Received: from scripts.lp.internal (scripts.lp.internal [10.131.215.246])
 (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
 key-exchange X25519 server-signature RSA-PSS (2048 bits) server-digest SHA256)
 (No client certificate requested)
 by smtp-relay-services-1.canonical.com (Postfix) with ESMTPSA id E430A3F817
 for <apparmor@lists.ubuntu.com>; Tue, 14 Jul 2026 23:00:44 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=launchpad.net;
 s=20210803; t=1784070045;
 bh=2QX4MR6+MekMhfPrLP8lYWm3K4F64gcoNB712QYHyrE=;
 h=MIME-Version:Content-Type:Date:From:To:Reply-To:References:
 Message-Id:Subject;
 b=
Received: from scripts.lp.internal (localhost [127.0.0.1])
 by scripts.lp.internal (Postfix) with ESMTP id 6E23A7E27E
 for <apparmor@lists.ubuntu.com>; Tue, 14 Jul 2026 23:00:44 +0000 (UTC)
MIME-Version: 1.0
Content-Type: text/plain; charset="utf-8"
Content-Transfer-Encoding: quoted-printable
Date: Tue, 14 Jul 2026 22:50:28 -0000
...
Message-Id: <178406942908.2915845.316592592093680040.launchpad@juju-98d295-prod-launchpad-3>
...
X-Launchpad-Hash: 427f4cdda5ce88362c9da6052a4d3bb7c8614f90
...
X-Spamd-Result: default: False [4.49 / 15.00];
	...
	DBL_BLOCKED_OPENRESOLVER(0.00)[lists.ubuntu.com:from_smtp,lists.ubuntu.com:helo,lists.ubuntu.com:rdns,launchpad.net:url,suse.com:url,bugs.launchpad.net:from_mime,bugs.launchpad.net:replyto,juju-98d295-prod-launchpad-3:mid]
X-Rspamd-Server: lfdr
X-Rspamd-Queue-Id: 0C72B7594F5

** Description changed:

+ [ Impact ]
+=20
+  * The following command should display all AVC denials: `ausearch --mess=
age AVC`; however, it doesn't work:
+ =20
+ ```
...
the AppArmor events, because the AppArmor is using event id `1400` (assigne=
d to SELinux) instead of `1500`.
+ =20
+  * AppArmor events don't have a `tclass` field, so `ausearch` treats ever=
y AppArmor event as 'malformed' and hides them. We can reveal the 'malforme=
d' events with the `--debug` flag:
+ =20
+  ```
...
+  ```
+ =20
+  * This bug makes `auditd` very difficult to use on Ubuntu: having the `a=
uditd` package installed hides AVC events from the kernel log AND `ausearch=
` doesn't reveal them, making AppArmor events seemingly vanish unless you k=
now where to look.
+ =20
+ [ Test plan ]
+=20
...
+=20
+  * This patch modifies the filtration behavior of `ausearch` by making th=
e `tclass` field optional, thus making AppArmor events be treated as well-f=
ormed SELinux events.
+ =20
+  * The regression risk with this patch is such that potentially well-form=
ed events will be hidden by `ausearch` unintentionally, or truly malformed =
events will be allowed past the filter.
+=20
...
+ [ Other info ]
+=20
+  * Debian likely doesn't want this patch, as Debian officially won't be m=
aking any changes until upstream does [3].
+ =20
+  * Upstream can't move the AppArmor event id to the `1500` range any time=
 soon. AppArmor was moved to the `1400` range to align with the Linux kerne=
l LSM infrastructure [2]
+ =20
+  * This patch comes from SUSE; they've been carrying it for well over a d=
ecade, and they seem to still be carrying it even after switching to SELinu=
x by default [3].
+=20
...
gi?bug=3D872726#88
+ [2] SUSE KB: https://support.scc.suse.com/s/kb/audit-log-file-that-has-th=
e-apparmor-AVC-entries-that-ausearch-can-t-read-1583239408721?language=3Den=
_US
+ [3] SUSE Patch: https://build.opensuse.org/projects/security/packages/aud=
...
"r" fsuid=3D0 ouid=3D0 FSUID=3D"root" OUID=3D"root"
  ...
  in the AppArmor events, because the AppArmor is using event id `1400`
  (assigned to SELinux) instead of `1500`.
